...
async function init() {
    ...
    parser.parse().then((rows) => {
        const parsedRows = rows.map(row => Object.entries(row).map((entry) => entry[1]));
        ...
        matchedRows.forEach(function (row) {
            ...
            $priceRowDiv.innerHTML = `<div>${data[1]}</div><div>${data[2]}</div><div title="${data[4] || data[3]}">${data[3]}</div>`
